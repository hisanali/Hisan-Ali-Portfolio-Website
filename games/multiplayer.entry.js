...
(() => {
  ...
  function renderGame(session) {
    ...
    if (game === 'connect') {
      ...
      arena.innerHTML = `<div class="memory-board">${state.cards.map((symbol,index)=>`<button type="button" class="memory-card${state.flipped.includes(index)||cardDone(state,index)?' is-flipped':''}${cardDone(state,index)?' is-matched':''}" data-card="${index}" aria-label="${state.flipped.includes(index)||cardDone(state,index)?(window.GameMemoryShapes?.name(symbol)||symbol):`Hidden card ${index+1}`}" ${!both||state.over||state.turn!==role||state.flipped.includes(index)||cardDone(state,index)||state.flipped.length>=2?'disabled':''}>${window.GameMemoryShapes?.svg(symbol)||symbol}</button>`).join('')}</div>`;
