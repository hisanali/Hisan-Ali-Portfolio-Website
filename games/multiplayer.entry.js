...
(() => {
  const supported = ['connect', 'reaction', 'memory', 'word', 'color'];
  const labels = {
  ...
  function modeMarkup(game) {
    return `<div class="multi-mode" role="group" aria-label="Choose ${labels[game]} mode">
      <button class="is-active" type="button" data-multi-mode="solo">Solo</button>
      <button type="button" data-multi-mode="online"><span></span>Online</button>
    </div>`;
  }
  ...
  async function openRoom(game, code, role, name, fresh = false) {
    ...
    const service = await client(); const panel=$(`[data-game-panel="${game}"]`); const native=panel.querySelector(':scope > :last-child');
