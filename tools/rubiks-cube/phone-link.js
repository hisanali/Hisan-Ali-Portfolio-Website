...
export async function connectPhone({token,role,onMessage,onStatus}){
 const response=await fetch('/api/multiplayer/config',{cache:'no-store'});if(!response.ok)throw Error('Phone pairing is unavailable. You can still scan on this device.');
 ...
 await new Promise((resolve,reject)=>{const timer=setTimeout(()=>{close();reject(Error('Pairing timed out. Check your connection and try again.'));},15000);
 channel.subscribe(s=>{if(closed)return;connected=s==='SUBSCRIBED';onStatus(connected?'Connected to pairing service.':'Reconnecting…');if(connected){clearTimeout(timer);resolve();}else if(s==='CHANNEL_ERROR')onStatus('Connection interrupted. Keep this page open to reconnect.');});});
 ...
 function close(){if(closed)return;closed=true;clearInterval(heartbeat);clearTimeout(expiry);client.removeChannel(channel);client.realtime.disconnect();}
