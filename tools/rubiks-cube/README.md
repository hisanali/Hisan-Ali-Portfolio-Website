...
- Validation for counts, piece combinations, corner twists, edge flips and parity. Immediately after scanning, the preview checks all 4,096 face rotations and applies the unique legal arrangement before displaying the finished cube. Ambiguous arrangements require a rescan, and sticker colours are never changed to force a solution.
...
- On phones, live rear-camera scanning uses one Start scanning tap. Faces can be shown in any order: the centre colour identifies the side automatically, captured/missing faces remain visible, and several steady full-face frames are averaged before acceptance. All 54 stickers are then classified together with an exact nine-per-colour constraint, preventing warm light from silently turning several orange stickers red or yellow stickers white. Only a face affected by glare or uncertainty is requested again. Desktop camera scanning retains photo upload, zoom, rotation and manual review as a fallback.
