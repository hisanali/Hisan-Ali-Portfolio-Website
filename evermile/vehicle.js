...
export class Vehicle{
 ...
 buildBike(){const m=new T.Group();this.group.add(m);this.bikeModel=m;
  ...
  const wheel=(z,front)=>{const w=new T.Group();w.position.set(0,.32,z);m.add(w);this.wheels.push(w);if(front)this.frontWheels.push(w);
   ...
   if(front)for(const x of [-.075,.075])add(new T.BoxGeometry(.03,.09,.07),gold,x*1.25,.11,-.1,0,0,0,w);return w;};
  wheel(.7,true);wheel(-.72,false);
  ...
  add(sphere,this.body,0,.98,.12).scale.set(.32,.22,.46);
  add(sphere,this.body,0,.95,.5).scale.set(.3,.28,.4);
  ...
  for(const x of [-.075,.075])add(block(.1,.05,.05,.02),this.white,x,1.0,.86,-.2);
  add(block(.28,.07,.42,.03),seat,0,1.06,-.24,.07);
  add(sphere,this.body,0,1.07,-.56,-.16).scale.set(.19,.13,.42);
  add(block(.12,.035,.03,.01),this.red,0,1.1,-.93,-.2);
  ...
  const head=add(new T.SphereGeometry(.145,28,20),helmet,0,1.56,.13,0,0,0,rider);head.scale.set(1,1,1.08);
  add(new T.SphereGeometry(.146,24,16,-Math.PI*.3,Math.PI*.6,Math.PI*.36,Math.PI*.22),visor,0,1.56,.135,0,0,0,rider);
  for(const s of [-1,1]){this.limb([s*.17,1.39,.02],[s*.23,1.2,.25],.05,suit,rider);this.limb([s*.23,1.2,.25],[s*.28,1.02,.41],.045,suit,rider);this.limb([s*.12,1.08,-.2],[s*.21,.9,.1],.075,suit,rider);this.limb([s*.21,.9,.1],[s*.2,.5,-.08],.06,suit,rider);add(block(.09,.07,.2,.03),seat,s*.2,.46,-.04,0,0,0,rider);}
 }
 ...
 update(dt,speed,steer,night,braking,firstPerson){if(this.detailed){for(const item of this.realWheels){item.object.quaternion.copy(item.base);if(item.front)item.object.rotateY(steer);item.object.rotateX(this.wheelAngle||0);}this.wheelAngle=(this.wheelAngle||0)-speed*dt/.34;for(const mat of this.realBrakeMats)mat.emissiveIntensity=braking?2.4:.45;this.detailed.visible=true;if(this.realSteer){this.realSteer.object.quaternion.copy(this.realSteer.base);this.realSteer.object.rotateY(steer*3.2);}if(this.realGlass)this.realGlass.opacity=firstPerson?.14:.43;const raw=dt>1e-4?(speed-(this.prevSpeed??speed))/dt:0;this.prevSpeed=speed;this.accel=(this.accel||0)+(Math.max(-15,Math.min(15,raw))-(this.accel||0))*Math.min(1,dt*3);this.pitch=(this.pitch||0)+(Math.max(-.022,Math.min(.022,-this.accel*.0022))-(this.pitch||0))*Math.min(1,dt*4);if(this.rig)this.rig.rotation.x=this.pitch;}for(const wheel of this.wheels){for(const part of wheel.children)part.rotation.x+=speed*dt/.39;}for(const wheel of this.frontWheels)wheel.rotation.y=steer;this.red.emissiveIntensity=braking?4:.7;this.white.emissiveIntensity=night?3:1;for(const light of this.headlights)light.intensity=night?100:0;this.interior.visible=firstPerson&&!this.detailed;if(this.type==='bike'){this.steering.rotation.y=steer*.55;if(this.rider)this.rider.visible=!firstPerson;}else this.steering.rotation.z=-steer*3.5;for(const child of this.group.children){if(child.userData.fallback)child.visible=!this.detailed&&!firstPerson;else if(child.isMesh)child.visible=!firstPerson;} }
