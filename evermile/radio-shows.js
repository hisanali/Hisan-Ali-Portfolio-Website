...
function road(c) {
  ...
  return ({
    ...
    farm: `Out on the farm lanes${name}? Keep an eye out for tractors, sheep and the odd very relaxed cow.`,
...
const TOPICS = [
  ...
  {title: 'I spy', lines: [['co', 'Right, road trip game. I spy with my little eye, something beginning with... R.'], ['host', 'Road.'], ['co', 'Oh, come on! Fine. Something beginning with S.'], ['host', 'Sky.'], ['co', 'You are the worst person to play this with.'], ['host', 'It\'s a road trip, Priya. It\'s always road, sky or sheep.']]},
...
export function eventLine(r, kind, d, c) {
  ...
  const line = {
    ...
    road: () => ({mountain: `We're heading up into the mountains now${d.name ? ', towards ' + d.name : ''}. Take the hairpins gently, and enjoy that view.`, highway: `Onto the motorway${d.name ? ', ' + d.name : ''}. Pick a lane, settle in, and let the kilometres roll by.`, coast: `Down to the lakeside road now${d.name ? ', towards ' + d.name : ''}. Windows down for that fresh air.`, farm: `Farm lanes from here${d.name ? ', out towards ' + d.name : ''}. Narrow roads, so watch for tractors and the odd sheep.`})[d.type] || `Back on the hill roads${d.name ? ', towards ' + d.name : ''}. Lovely.`,
    junction: () => d.dir === 'ahead' ? pick(['Straight on at the junction. Good choice.', 'Keeping straight on. The road knows the way.']) : `Taking the turn for the ${d.label}${d.name ? ', towards ' + d.name : ''}? ${pick(['Ooh, adventurous. I like it.', 'Good call, it\'s beautiful that way.', 'Excellent choice.'])}`,
